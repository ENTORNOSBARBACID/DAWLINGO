...
export class LeccionesComponent {
  ...
  subirNivel(idUsu: number, idCurso: number) {
    this.login.sumarPuntosNivel(this.idUsu).subscribe({
      next: (respuesta) => {
        ...
        this.data.UpdateNivelUsuarioCurso(idUsu, idCurso).subscribe({
          next: (res: any) => {
            ...
            alert(
              '¡Has completado todas las lecciones del nivel! Se han sumado 100 puntos.'
            );
            this.recargarDatos(); // Recarga los datos para reflejar el progreso
          },
          error: (err) => {
  ...
  atras() {
    ...
    this.route.navigate(['/home/curso-niveles/' + this.idNivel]);
  }
}
